...
from .featurization_mat_utils import (
    add_dummy_node,
...
class MatFeaturizer(
    PretrainedFeaturizerMixin[MatMoleculeEncoding, MatBatchEncoding, MatConfig]
):
    @classmethod
    ...
    def _encode_smiles(self, smiles: str, y: Optional[float]) -> MatMoleculeEncoding:
        ...
        generated_features = generate_additional_features(
            mol, self.config.generator_features_generators
        )

        node_features, adj_matrix, dist_matrix, _ = add_dummy_node(
            node_features=node_features, adj_matrix=adj_matrix, dist_matrix=dist_matrix
        )

        return MatMoleculeEncoding(
    ...
    def _collate_encodings(
        ...
        adj_matrix = pad_sequence(
            [torch.tensor(e.adjacency_matrix).float() for e in encodings]
        )
        dist_matrix = pad_sequence(
            [torch.tensor(e.distance_matrix).float() for e in encodings]
        )

        return MatBatchEncoding(
            node_features=node_features,
            adjacency_matrix=adj_matrix,
            distance_matrix=dist_matrix,
            y=stack_y(encodings),
