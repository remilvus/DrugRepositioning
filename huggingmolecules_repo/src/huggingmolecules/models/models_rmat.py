...
class RMatModel(PretrainedModelBase[RMatBatchEncoding, RMatConfig]):
    ...
    def __init__(self, config: RMatConfig):
        ...
        self.src_embed = Embedding(
            d_input=config.d_atom, d_output=config.d_model, dropout=config.dropout
        )

        # Distance
        self.dist_rbf = BesselBasisLayerEnvelope(
            num_radial=config.envelope_num_radial,
        ...
        sa_layer = MultiHeadedAttention(
            h=config.encoder_n_attn_heads,
...
class BesselBasisLayerEnvelope(nn.Module):
    def __init__(self, *, num_radial: float, cutoff: float, exponent: float):
        super().__init__()
        self.num_radial = num_radial
        self.cutoff = cutoff
        self.sqrt_cutoff = np.sqrt(2.0 / cutoff)
    ...
    def forward(self, inputs):
        ...
        frequencies = (
            np.pi * torch.arange(1, self.num_radial + 1, device=inputs.device).float()
        )
...
class RMatEdgeFeaturesLayer(nn.Module):
    ...
    def forward(self, p_edge):
        ...
        p_edge = p_edge.view(
            p_edge.size(0), self.h, self.d_k, p_edge.size(2), p_edge.size(3)
        )
        return p_edge
