...
import src.huggingmolecules_repo.models as huggingmolecules_models
from src.huggingmolecules_repo.configuration.configuration_api import (
    PretrainedConfigMixin,
)
from src.huggingmolecules_repo.downloading.downloading_utils import (
    HUGGINGMOLECULES_CACHE,
)
...
from .training_callbacks import (
    NeptuneCompatibleCallback,
    GinConfigSaver,
    ModelConfigSaver,
)
from ..gin.gin_utils import (
    get_formatted_config_str,
    parse_gin_str,
    get_default_experiment_name,
)

default_cache_dir = os.path.join(HUGGINGMOLECULES_CACHE, "encodings")
HUGGINGMOLECULES_ENCODINGS_CACHE = os.getenv(
    "HUGGINGMOLECULES_ENCODINGS_CACHE", default_cache_dir
)
...
class GinModel:
    def __init__(self, cls_name: str, pretrained_name: str, **kwargs):
...
def get_default_callbacks() -> List[Callback]:
    gin_config_essential = GinConfigSaver(
        target_name="gin-config-essential.txt",
        excluded_namespaces=["neptune", "optuna", "macro", "benchmark"],
    )
    return [gin_config_essential, ModelConfigSaver(), GinConfigSaver()]
...
def get_all_hyperparams(model: PretrainedModelBase) -> Dict[str, Any]:
    gin_str = get_formatted_config_str(
        excluded=["neptune", "optuna", "macro", "benchmark"]
    )
...
def get_data_loaders(
    ...
    **kwargs,
    ...
    if cache_encodings and _encodings_cached():
        ...
        split["train"]["X"] = featurizer.encode_smiles_list(
            split["train"]["X"], split["train"]["Y"]
        )
        split["valid"]["X"] = featurizer.encode_smiles_list(
            split["valid"]["X"], split["valid"]["Y"]
        )
        split["test"]["X"] = featurizer.encode_smiles_list(
    ...
    valid_loader = featurizer.get_data_loader(
...
def get_data_split(
    ...
    if dataset_path is None:
        split = _get_data_split_from_tdc(
            task_name, dataset_name, assay_name, split_method, split_frac, split_seed
        )
    else:
        split = _get_data_split_from_csv(
            dataset_name, assay_name, dataset_path, split_method, split_frac, split_seed
        )
...
def _get_data_split_from_csv(
    ...
    if os.path.exists(split_path):
        ...
        train_data, valid_data, test_data = _split_data_random(
            data, split_frac, split_seed
        )
    else:
...
def _split_data_random(data, split_frac: Tuple[float, float, float], seed: int = None):
    ...
    train_data, valid_data, test_data = random_split(
        data, [train_len, valid_len, test_len], generator=generator
    )
    return (
        data.iloc[train_data.indices],
        data.iloc[valid_data.indices],
        data.iloc[test_data.indices],
    )
...
def normalize_labels_inplace(split: Split) -> None:
    ...
    scaler = preprocessing.StandardScaler().fit(
        np.concatenate([split["train"]["Y"], split["valid"]["Y"]]).reshape(-1, 1)
    )
    split["train"]["Y"] = scaler.transform(split["train"]["Y"].reshape(-1, 1)).reshape(
        -1
    )
    split["valid"]["Y"] = scaler.transform(split["valid"]["Y"].reshape(-1, 1)).reshape(
        -1
    )
