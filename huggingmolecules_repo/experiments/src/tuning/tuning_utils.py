...
class Objective:
    ...
    def __call__(self, trial: optuna.trial.Trial) -> float:
        ...
        suggested_values = {
            p: getattr(trial, v[0])(p, *v[1:]) for p, v in self.optuna_params.items()
        }
        bind_parameters_from_dict(suggested_values)

        model = copy.deepcopy(self.model) if self.model else None
        trainer = train_model(
            model=model, featurizer=self.featurizer, root_path=trial_path
        )
...
def enqueue_failed_trials(study, retry_not_completed: bool) -> None:
    ...
    if retry_not_completed:
        condition = lambda trial: trial.state not in [
            TrialState.COMPLETE,
            TrialState.WAITING,
        ]
    else:
