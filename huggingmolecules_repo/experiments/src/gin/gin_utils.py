...
def get_default_experiment_name(
    ...
    dataset_name = (
        dataset_name if dataset_name else gin.query_parameter("data.dataset_name")
    )
    try:
        assay_name = (
            assay_name if assay_name else gin.query_parameter("data.assay_name")
        )
...
def get_formatted_config_str(excluded: Optional[List[str]] = None) -> str:
    ...
    if excluded:
        config_map = {
            k: v for k, v in config_map.items() if all(x not in k[1] for x in excluded)
        }
