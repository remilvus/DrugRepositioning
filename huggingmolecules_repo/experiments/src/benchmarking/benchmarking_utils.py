...
def create_grid_results_dict() -> GridResultDict:
    ...
    return {
        frozenset(hps.items()): {seed: None for seed in seeds}
        for hps in hps_product_list
    }
...
def download_dataframe_from_neptune(neptune_project, name: str) -> pd.DataFrame:
    ...
    for param_name, param_value in params_dict.items():
        ...
        df[param_name] = (
            df[param_name].astype(dtype)
            if dtype != int
            else df[param_name].astype(float).astype(int)
        )
    ...
    for path in os.listdir(save_path):
        ...
        if os.path.isdir(trial_path):
            ...
            results = load_results_from_local(
                os.path.join(experiment_path, "results.json")
            )
    ...
    for params, results in grid_results_dict.items():
        for seed, result in results.items():
            if result is None:
                ...
                logging.error(
                    f"Results for seed: {seed} and hps: {dict(params)} are missing"
                )
...
def compute_result(
    ...
    results = [
        (params, average_dictionary(results))
        for params, results in grid_results_dict.items()
    ]
