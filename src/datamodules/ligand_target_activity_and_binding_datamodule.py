...
def merge_collate_fns(ligand_collate_fn, target_collate_fn):
    def collate_fn(datapoints: list[DataPoint]):
        ...
        targets_features = target_collate_fn([datapoint.target_features for datapoint in datapoints])
        ligands_features = ligand_collate_fn([datapoint.ligand_features for datapoint in datapoints])

        activities = torch.stack([torch.tensor(datapoint.activity).float() for datapoint in datapoints])
        binding_scores = torch.stack([torch.tensor(datapoint.binding_score).float() for datapoint in datapoints])

        return DataBatch(ligands, ligands_features, targets, targets_features, activities, binding_scores)
...
class LigandTargetActivityAndBindingDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: str):
        ...
        train_ligands, test_ligands = train_test_split(all_ligands, test_size=self.test_size)
        train_ligands, val_ligands = train_test_split(train_ligands, test_size=self.val_size)
        train_targets, test_targets = train_test_split(all_targets, test_size=self.test_size)
        train_targets, val_targets = train_test_split(train_targets, test_size=self.val_size)

        train_ligands, val_ligands, test_ligands = set(train_ligands), set(val_ligands), set(test_ligands)
        train_targets, val_targets, test_targets = set(train_targets), set(val_targets), set(test_targets)

        if stage == 'fit':
            self.train_dataset = LigandTargetActivityAndBinding(train_ligands,
                                                                train_targets,
                                                                self.ligand_featurizer,
                                                                self.target_featurizer)
            self.val_dataset = LigandTargetActivityAndBinding(val_ligands,
                                                              val_targets,
                                                              self.ligand_featurizer,
                                                              self.target_featurizer)
        if stage == 'test':
            self.test_dataset = LigandTargetActivityAndBinding(test_ligands,
                                                               test_targets,
                                                               self.ligand_featurizer,
                                                               self.targer_featurizer)
        if stage == 'predict':
            self.predict_dataset = LigandTargetActivityAndBinding(test_ligands,
                                                                  test_targets,
                                                                  self.ligand_featurizer,
                                                                  self.target_featurizer)
