...
class SchnetFeaturizer(Featurizer):
    def get_features(self):
        ...
        for i, atom in enumerate(mol.GetAtoms()):
            node_z.append(atom.GetAtomicNumber())
    ...
    def collate_fn(encodings):
        ...
        repeat_counts = [encoding.size for encoding in encodings]
        batch = torch.from_numpy(np.repeat(np.arange(len(encodings)), repeat_counts))
